// Derived from Monado openxr sample
...
render_frame(int w,
             ...
             GLuint depthbuffer, bool left_clicked, bool right_clicked);
...
main(int argc, char** argv)
{
	...
	if (init_gl(view_count, swapchain_lengths, &gl_rendering.framebuffers) != 0) {
	...
	while (!quit_mainloop) {
		...
		bool right_clicked = grab_value[1].isActive && grab_value[1].currentState > 0.75;
		...
		for (uint32_t i = 0; i < view_count; i++) {
			...
			render_frame(w, h, frame_state.predictedDisplayTime, i, hand_locations, projection_matrix,
			             view_matrix, gl_rendering.framebuffers[i][acquired_index],
			             images[i][acquired_index].image, depth.supported, depth_image, left_clicked, right_clicked);
...
#endif
{
	...
	desktop_window =
	    SDL_CreateWindow("OpenXR Example", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, w / 4,
	                     h / 4, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
	if (!desktop_window) {
...
//	glClearColor(.0f, 0.0f, 0.2f, 1.0f);
	...
	float leftHandPosition[3];
	float leftHandOrientation[4];
	float rightHandPosition[3];
	float rightHandOrientation[4];
	if (hand_locations[0].locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) {
		leftHandPosition[0] = hand_locations[0].pose.position.x;
		leftHandPosition[1] = hand_locations[0].pose.position.y;
		leftHandPosition[2] = hand_locations[0].pose.position.z;
	}
	if (hand_locations[0].locationFlags & XR_SPACE_LOCATION_ORIENTATION_VALID_BIT) {
		leftHandOrientation[0] = hand_locations[0].pose.orientation.x;
		leftHandOrientation[1] = hand_locations[0].pose.orientation.y;
		leftHandOrientation[2] = hand_locations[0].pose.orientation.z;
		leftHandOrientation[3] = hand_locations[0].pose.orientation.w;
	}
	if (hand_locations[1].locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) {
		rightHandPosition[0] = hand_locations[1].pose.position.x;
		rightHandPosition[1] = hand_locations[1].pose.position.y;
		rightHandPosition[2] = hand_locations[1].pose.position.z;
	}
	if (hand_locations[1].locationFlags & XR_SPACE_LOCATION_ORIENTATION_VALID_BIT) {
		rightHandOrientation[0] = hand_locations[1].pose.orientation.x;
		rightHandOrientation[1] = hand_locations[1].pose.orientation.y;
		rightHandOrientation[2] = hand_locations[1].pose.orientation.z;
		rightHandOrientation[3] = hand_locations[1].pose.orientation.w;
	}



       	if (view_index == 0) {
	  vectorioXRUpdate(16, projectionmatrix.m, viewmatrix.m, leftHandPosition, leftHandOrientation, rightHandPosition, rightHandOrientation, left_clicked, right_clicked);
		} else {
        }
	vectorioXRRender(projectionmatrix.m, viewmatrix.m);
